...
window.vis_targetingStrategies = function(container, lang) {
  ...
  const strat = [
    { icon:'📢', reach:pt?'MERCADO TODO':'WHOLE MARKET', color:'#0C4A6E', name:pt?'Indiferenciado (Massa)':'Undifferentiated (Mass)', desc:pt?'Mira o mercado inteiro com UMA única oferta, focando no que é COMUM a todos, não nas diferenças. Também chamado de marketing de massa. É o mais barato. Ex: commodities (sal, açúcar), Coca-Cola clássica; a IKEA usa uma oferta ampla pra quase todo mundo.':'Targets the whole market with ONE single offer, focusing on what is COMMON to everyone, not on differences. Also called mass marketing. It is the cheapest. E.g. commodities (salt, sugar), classic Coca-Cola; IKEA uses one broad offer for almost everyone.' },
    { icon:'🎯', reach:pt?'VÁRIOS SEGMENTOS':'SEVERAL SEGMENTS', color:'#0284C7', name:pt?'Diferenciado':'Differentiated', desc:pt?'Mira VÁRIOS segmentos e desenha uma oferta SEPARADA pra cada um. Objetivo: mais vendas e posição mais forte. É relativamente CARO (vários produtos e campanhas). Ex: o Premier Inn com “Premium” (casais/negócios) + “Longer stays” (famílias); a Nike faz linhas separadas pra corredor, levantador, ciclista, golfista.':'Targets SEVERAL segments and designs a SEPARATE offer for each. Goal: higher sales and a stronger position. It is relatively EXPENSIVE (several products and campaigns). E.g. Premier Inn with “Premium” (couples/business) + “Longer stays” (families); Nike runs separate lines for runners, weightlifters, cyclists, golfers.' },
    { icon:'🔬', reach:pt?'UM NICHO':'ONE NICHE', color:'#0891B2', name:pt?'Concentrado (Nicho)':'Concentrated (Niche)', desc:pt?'Vai atrás de uma GRANDE fatia de UM (ou poucos) segmento pequeno. Também chamado marketing de nicho. Domina o nicho em vez de disputar o mercado todo. Ex: Ferrari, Rolex; a Lush (cosmético ético) domina o nicho vegano/sustentável.':'Goes after a LARGE share of ONE (or a few) small segment. Also called niche marketing. Dominates the niche instead of fighting for the whole market. E.g. Ferrari, Rolex; Lush (ethical cosmetics) dominates the vegan/sustainable niche.' },
    { icon:'📍', reach:pt?'LOCAL / 1-A-1':'LOCAL / 1-TO-1', color:'#16A34A', name:pt?'Micromarketing':'Micromarketing', desc:pt?'Molda produtos e programas pro gosto de indivíduos e locais específicos. Inclui marketing local (por bairro/loja) e marketing individual (1-a-1, personalizado). Ex: cardápio que muda por bairro; o Groupon entrega ofertas hiperlocais personalizadas por usuário.':'Tailors products and programmes to the taste of specific individuals and locations. Includes local marketing (by neighbourhood/store) and individual marketing (1-to-1, personalised). E.g. a menu that changes by neighbourhood; Groupon delivers hyper-local deals personalised per user.' }
  ];
...
/* ─── MKT W3: DAMAS test — 5 criteria to evaluate a target segment ─ */
window.vis_damasTest = function(container, lang) {
  const pt = lang === 'pt';
  const id = 'damas-' + Math.random().toString(36).substr(2,5);
  const crit = [
    { L:'D', color:'#0EA5E9', name:pt?'Distinto':'Distinct', ask:pt?'Se comporta diferente?':'Behaves differently?', desc:pt?'O segmento se comporta de forma diferente dos outros e merece uma abordagem separada. Se dois “segmentos” respondem igual à mesma oferta, não são distintos — junte-os num só.':'The segment behaves differently from other groups and warrants a separate approach. If two “segments” respond the same to the same offer, they are not distinct — merge them.' },
    { L:'A', color:'#0284C7', name:pt?'Acessível':'Accessible', ask:pt?'Você consegue alcançar?':'Can you reach it?', desc:pt?'Dá pra alcançá-lo pelos canais de mídia, distribuição e vendas disponíveis. Um segmento que você não consegue atingir não serve, por melhor que pareça no papel.':'You can reach it through available media, distribution and sales channels. A segment you cannot reach is useless, however good it looks on paper.' },
    { L:'M', color:'#0891B2', name:pt?'Mensurável':'Measurable', ask:pt?'Dá pra medir?':'Can you quantify it?', desc:pt?'Tamanho, poder de compra e comportamento podem ser quantificados. Sem medir, você não planeja nem prova o retorno do investimento.':'Its size, purchasing power and behaviour can be quantified. Without measuring, you cannot plan or prove the return on investment.' },
    { L:'A', color:'#7C3AED', name:pt?'Acionável':'Actionable', ask:pt?'Você tem capacidade?':'Do you have the capability?', desc:pt?'Sua empresa tem as capacidades E o orçamento pra servir esse segmento de forma LUCRATIVA. Liga direto com a regra de ouro do SWOT (Sem. 12): uma oportunidade só vale se você tem força interna pra agarrá-la.':'Your organisation has the capabilities AND the budget to serve it PROFITABLY. Ties straight to the SWOT golden rule (Week 12): an opportunity only counts if you have the internal strength to seize it.' },
    { L:'S', color:'#16A34A', name:pt?'Substancial':'Substantial', ask:pt?'É grande/valioso o bastante?':'Big/valuable enough?', desc:pt?'O segmento é grande o suficiente — ou valioso o suficiente POR CLIENTE — pra justificar o investimento. Um nicho pequeno pode valer a pena se cada cliente paga muito (ex: Ferrari).':'The segment is large enough — or valuable enough PER CUSTOMER — to justify the investment. A small niche can be worth it if each customer pays a lot (e.g. Ferrari).' }
  ];
  window[id+'_data'] = crit;
  window[id+'_sel'] = function(i){ const c=window[id+'_data'][i]; const b=document.getElementById(id+'-b'); if(!b)return; document.querySelectorAll('.'+id+'-c').forEach(function(x){x.style.outline='none';}); document.getElementById(id+'-c'+i).style.outline='2.5px solid '+c.color; document.getElementById(id+'-n').textContent=c.L+' · '+c.name; document.getElementById(id+'-n').style.color=c.color; document.getElementById(id+'-a').textContent=c.ask; document.getElementById(id+'-a').style.background=c.color; document.getElementById(id+'-d').textContent=c.desc; b.style.display='block'; };
  container.innerHTML = `
    <div style="padding:4px;font-family:sans-serif;">
      <div style="font-size:9px;text-transform:uppercase;letter-spacing:.7px;font-weight:700;color:#0C4A6E;margin-bottom:2px;">
        ${pt?'O Teste DAMAS — vale a pena mirar esse segmento?':'The DAMAS Test — is this segment worth targeting?'}
      </div>
      <div style="font-size:7.5px;color:#64748B;margin-bottom:8px;">${pt?'5 filtros antes de gastar 1 real num segmento. Clique em cada um.':'5 filters before spending a cent on a segment. Click each one.'}</div>
      <div style="display:flex;flex-direction:column;gap:4px;">
        ${crit.map((c,i)=>`
          <div id="${id}-c${i}" class="${id}-c" onclick="window['${id}_sel'](${i})" style="display:flex;align-items:center;gap:8px;background:${c.color}0D;border:1px solid ${c.color}33;border-radius:6px;padding:6px 9px;cursor:pointer;transition:outline .12s;">
            <span style="flex:none;width:21px;height:21px;border-radius:5px;background:${c.color};color:#fff;font-size:11px;font-weight:900;display:flex;align-items:center;justify-content:center;">${c.L}</span>
            <span style="font-size:8.5px;font-weight:800;color:${c.color};min-width:66px;">${c.name}</span>
            <span style="font-size:7.5px;color:#64748B;font-weight:600;">${c.ask}</span>
          </div>`).join('')}
      </div>
      <div id="${id}-b" style="display:none;margin-top:8px;background:#F8FAFC;border:1px solid #E5E7EB;border-radius:7px;padding:9px 11px;">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;">
          <span id="${id}-n" style="font-size:10px;font-weight:800;"></span>
          <span id="${id}-a" style="font-size:6.5px;font-weight:700;color:#fff;padding:2px 7px;border-radius:10px;"></span>
        </div>
        <div id="${id}-d" style="font-size:8.5px;color:#374151;line-height:1.6;"></div>
      </div>
      <div style="margin-top:8px;background:#EFF6FF;border-radius:6px;padding:6px 9px;font-size:7.5px;color:#1E40AF;line-height:1.55;">
        💡 ${pt?'<b>DAMAS</b> (“as damas” 👑): reprovou em QUALQUER um dos 5 → repensa o segmento. Evita perseguir alvo que é lindo no papel mas impossível de converter.':'<b>DAMAS</b>: fail ANY of the 5 → rethink the segment. It stops you chasing a target that looks great on paper but is impossible to convert.'}
      </div>
    </div>`;
};
...
/* ─── MKT W8: Berman & Thelen 4-level pathway multi→omni (from the real paper) ─── */
window.vis_omniTransition = function(container, lang) {
  const pt = lang === 'pt';
  const id = 'otr-' + Math.random().toString(36).substr(2,5);
  const lv = [
    { n:'1', tag:pt?'≈ Crawl / Silo':'≈ Crawl / Silo', color:'#7DD3FC', name:pt?'Multichannel em silo':'Siloed multichannel', desc:pt?'Programa multichannel convencional: cada canal é planejado e operado SEPARADAMENTE e COMPETE com os outros por venda e lealdade. Sortimento, preço, marca e promoção variam por canal; cupom não vale entre canais. Sem coordenação nem dados compartilhados. É o ponto de partida.':'A conventional multichannel program: each channel is planned and run SEPARATELY and COMPETES with the others for sales and loyalty. Assortment, price, brand and promotion vary by channel; a coupon is not valid across channels. No coordination, no shared data. The starting point.' },
    { n:'2', tag:pt?'≈ Integração mínima':'≈ Minimal integration', color:'#38BDF8', name:pt?'Imagem uniforme':'Uniform image', desc:pt?'Primeira integração, só na APARÊNCIA: logos, cores, fontes e descrições de produto padronizados entre canais e dispositivos. Cria um visual comum — mas os DADOS do cliente ainda NÃO são compartilhados e cada canal continua com sua própria equipe. Ex: cupom no celular que convida a ir à loja.':'The first integration, only in APPEARANCE: standardized logos, colours, fonts and product descriptions across channels and devices. It creates a common look — but customer DATA is still NOT shared and each channel still has its own team. E.g. a mobile coupon inviting a store visit.' },
    { n:'3', tag:pt?'≈ Walk / Run':'≈ Walk / Run', color:'#0EA5E9', name:pt?'Pedido e devolução integrados':'Integrated ordering & returns', desc:pt?'Integração operacional real: pedido, devolução e acesso à info do cliente cruzam os canais. Nasce o BOPIS (compra online, retira na loja), BORIS (devolve na loja), BOSS/BOSH. Estoque unificado: o cliente vê no site o que tem na loja. O BOPIS mata o atrito nº1 (será que tem? quanto custa?).':'Real operational integration: ordering, returns and customer-info access cross the channels. BOPIS is born (buy online, pick up in store), BORIS (return in store), BOSS/BOSH. Unified inventory: the customer sees online what the store holds. BOPIS kills the nº1 friction (is it in stock? what does it cost?).' },
    { n:'4', tag:pt?'≈ Sprint / Full':'≈ Sprint / Full', color:'#0C4A6E', name:pt?'Omnichannel verdadeiro':'True omnichannel', desc:pt?'O topo: promoção e preço comuns, UMA única fidelidade entre canais, histórico de compra compartilhado, jornada sem costura e sinergias entre canais (o anúncio online leva à loja e vice-versa). Exige INCENTIVOS cross-channel em toda a organização de marketing. Ex: Williams-Sonoma, base ligada a 70% das compras, 11 cientistas de dados.':'The top: common promotions and price, ONE loyalty program across channels, shared purchase history, a seamless journey and cross-channel synergies (the online ad drives store visits and vice versa). It requires cross-channel INCENTIVES across the whole marketing organisation. E.g. Williams-Sonoma, a database tied to 70% of purchases, 11 data scientists.' }
  ];
  window[id+'_data'] = lv;
  window[id+'_sel'] = function(i){ const s=window[id+'_data'][i]; const bx=document.getElementById(id+'-b'); if(!bx)return; document.querySelectorAll('.'+id+'-c').forEach(function(x){x.style.outline='none';}); document.getElementById(id+'-c'+i).style.outline='2.5px solid '+s.color; document.getElementById(id+'-n').textContent=(pt?'Nível ':'Level ')+s.n+' · '+s.name; document.getElementById(id+'-n').style.color=(i===3?'#0C4A6E':'#0369A1'); document.getElementById(id+'-t').textContent=s.tag; document.getElementById(id+'-d').textContent=s.desc; bx.style.display='block'; };
  container.innerHTML = `
    <div style="padding:4px;font-family:sans-serif;">
      <div style="font-size:9px;text-transform:uppercase;letter-spacing:.7px;font-weight:700;color:#0C4A6E;margin-bottom:2px;">
        ${pt?'Migrar de multi → omni: os 4 níveis':'Multi → omni: the 4 levels'}
      </div>
      <div style="font-size:7.5px;color:#64748B;margin-bottom:8px;">${pt?'O modelo real de Berman & Thelen (2018). Sobe UM degrau por vez. Clique.':'Berman & Thelen\'s (2018) actual model. You climb ONE step at a time. Click.'}</div>
      <div style="display:flex;flex-direction:column;gap:4px;">
        ${lv.map((s,i)=>`<div id="${id}-c${i}" class="${id}-c" onclick="window['${id}_sel'](${i})" style="display:flex;align-items:center;gap:8px;background:${s.color}1A;border:1px solid ${s.color};border-radius:6px;padding:6px 9px;cursor:pointer;transition:outline .12s;"><span style="flex:none;width:19px;height:19px;border-radius:5px;background:${s.color};color:${i===3?'#fff':'#0C4A6E'};font-size:10px;font-weight:900;display:flex;align-items:center;justify-content:center;">${s.n}</span><span style="font-size:8.5px;font-weight:800;color:${i===3?'#0C4A6E':'#0369A1'};flex:1;">${s.name}</span><span style="font-size:6px;font-weight:700;color:#94A3B8;">${s.tag}</span></div>`).join('')}
      </div>
      <div id="${id}-b" style="display:none;margin-top:8px;background:#F8FAFC;border:1px solid #E5E7EB;border-radius:7px;padding:9px 11px;">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;">
          <span id="${id}-n" style="font-size:10px;font-weight:800;"></span>
          <span id="${id}-t" style="font-size:6.5px;font-weight:700;color:#94A3B8;"></span>
        </div>
        <div id="${id}-d" style="font-size:8.5px;color:#374151;line-height:1.6;"></div>
      </div>
      <div style="margin-top:8px;background:#EFF6FF;border-radius:6px;padding:6px 9px;font-size:7.5px;color:#1E40AF;line-height:1.6;">💡 ${pt?'Regra: <b>não dá pra pular nível</b> (nem com dinheiro) — trocar o comportamento channel-centric por um modelo <b>customer-centric e channel-agnostic</b>. Barreira real = silos + metas/incentivos descasados, NÃO a tecnologia. (O crawl/walk/run/sprint é de Strang 2013, que eles citam.)':'Rule: <b>you cannot skip a level</b> (not even with money) — replace channel-centric behaviour with a <b>customer-centric, channel-agnostic</b> model. The real barrier = silos + mismatched targets/incentives, NOT technology. (Crawl/walk/run/sprint is Strang 2013, whom they cite.)'}</div>
    </div>`;
};
